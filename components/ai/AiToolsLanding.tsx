...
/* /ai-tools landing: premium hero, instant search, category filter,
   popular & new rails, full grid — all from lib/ai-tools-meta. */

import Link from "next/link";
import { useMemo, useState } from "react";
import { FiSearch, FiArrowRight, FiZap } from "react-icons/fi";
import { AI_TOOLS, AI_CATEGORIES, type AiCategory } from "@/lib/ai-tools-meta";

function ToolCard({ t, i }: { t: (typeof AI_TOOLS)[number]; i: number }) {
  return (
    <Link href={`/ai-tools/${t.slug}`}
...
export default function AiToolsLanding() {
  ...
  const fresh = AI_TOOLS.filter((t) => t.isNew);
  const liveCount = AI_TOOLS.filter((t) => t.status === "live").length;
    ...
    <main className="max-w-[1400px] mx-auto px-5 lg:px-8 py-12 lg:py-16">
      ...
      <div className="text-center mb-12 relative">
        <div className="absolute inset-x-0 -top-10 h-64 pointer-events-none" aria-hidden
          style={{ background: "radial-gradient(600px circle at 50% 0%, color-mix(in srgb, var(--accent) 12%, transparent), transparent 70%)" }} />
        <span className="qx-badge-hero inline-flex mb-5 relative">
          <FiZap size={13} style={{ marginRight: 6 }} /> {liveCount} tools live · on-device AI · free forever
        </span>
        <h1 className="font-display text-4xl lg:text-6xl font-extrabold relative" style={{ color: "var(--text)" }}>
          The <span className="qx-aurora">AI toolbox</span> in your browser
        </h1>
        <p className="mt-4 text-[15px] max-w-xl mx-auto relative" style={{ color: "var(--text-muted)" }}>
          Remove backgrounds, upscale photos, read text, build logos and resumes — private, instant, no signup.
        </p>
        ...
        <div className="mt-8 max-w-xl mx-auto relative">
          <div className="flex items-center gap-3 rounded-full px-5 py-3.5"
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search AI tools…"
              className="flex-1 bg-transparent outline-none text-[14.5px]" style={{ color: "var(--text)" }} aria-label="Search AI tools" />
          </div>
        </div>
        <div className="mt-5 flex flex-wrap justify-center gap-2 relative">
          {(["All", ...AI_CATEGORIES] as const).map((c) => (
      ...
      {/* rails (hidden while searching/filtering) */}
      {!q && cat === "All" && (
        <>
          <section className="mb-10" aria-label="Popular AI tools">
          ...
          {fresh.length > 0 && (
            <section className="mb-10" aria-label="New AI tools">
      ...
      {/* full grid */}
      <section aria-label="All AI tools">
        <h2 className="font-display text-xl font-bold mb-4" style={{ color: "var(--text)" }}>
          {q || cat !== "All" ? `Results (${filtered.length})` : `All AI tools (${AI_TOOLS.length})`}
        </h2>
        {filtered.length === 0 ? (
