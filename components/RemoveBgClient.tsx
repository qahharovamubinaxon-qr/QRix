...
/* 10 background colours (the first is transparent). `key` stays English and
   ASCII because it is ALSO the download filename suffix — localizing it would
   put Cyrillic in a saved file's name. Only the visible/announced label is
   translated, out of lib/tool-ui-i18n. */
const BG_COLORS = [
...
export default function RemoveBgClient({ lang = "en" }: { lang?: ToolLang }) {
  const t = toolUI(lang);
  ...
  const remove = async () => {
    if (!file) return;
    setBusy(true);
    setStatus(t.removeBg.loadingModel(""));
    try {
      ...
      const blob = await removeBackground(file, {
        progress: (key: string, current: number, total: number) => {
          if (key.includes("fetch")) {
            ...
            setStatus(t.removeBg.loadingModel(pct));
          } else {
            setStatus(t.removeBg.processingPct(Math.round((current / total) * 100)));
          }
        },
      });
  ...
  const download = () => {
    ...
    a.href = src;
    const suffix = bgColor === "transparent" ? "no-bg" : "bg-" + BG_COLORS.find((c) => c.value === bgColor)?.key;
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <div className="qx-card p-6">
        <h3 className="font-display text-sm font-bold mb-4" style={{ color: "var(--text)" }}>{t.removeBg.original}</h3>
        <label
        ...
        >
          {original ? (
            ...
            <>
              ...
              <span className="text-sm font-semibold" style={{ color: "var(--text)" }}>{t.common.chooseImage}</span>
        ...
        <button onClick={remove} disabled={!file || busy} className="qx-btn w-full mt-5 !py-3.5 disabled:opacity-50">
          {busy ? <><FiLoader size={15} className="animate-spin" /> {status || t.removeBg.processing}</> : <><FiScissors size={15} /> {t.removeBg.removeBtn}</>}
        </button>
        {busy && (
          <p className="mt-2 text-[11px] text-center" style={{ color: "var(--text-faint)" }}>
            {t.removeBg.localNote}
          </p>
        )}
      </div>
      ...
      <div className="qx-card p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display text-sm font-bold" style={{ color: "var(--text)" }}>
            {t.removeBg.result} {bgColor === "transparent" ? t.removeBg.transparentTag : ""}
          </h3>
          {cutout && (
        ...
        <div className="rounded-2xl min-h-[280px] flex items-center justify-center p-4"
          ...
          {shown ? (
            ...
            <div className="text-center" style={{ color: "var(--text-faint)" }}>
              ...
              <p className="text-xs">{t.removeBg.resultHere}</p>
            </div>
          )}
        </div>
          ...
          <div className="mt-5">
            ...
            <div className="flex flex-wrap gap-2">
              {BG_COLORS.map((c) => {
                ...
                const name = t.removeBg.colors[c.key];
                return (
                  <button
                    key={c.value}
