...
export async function generateMetadata({ params }: { params: Promise<{ tool: string }> }) {
  ...
  if (!t) return {};
  return pageMeta({
    title: t.ru.title, description: t.ru.desc, path: `/ru/${tool}`, keywords: t.ru.keywords,
...
export default async function Page({ params }: { params: Promise<{ tool: string }> }) {
  ...
  const t = getLocTool(tool);
  if (!t) notFound();
  return <LocalizedToolPage tool={t} lang="ru" others={LOC_TOOLS.filter((x) => x.slug !== tool)} />;
}
