...
function fmtAnswer(engine: string): string {
  if (engine.startsWith("batch:")) {
    ...
    if (p === "meta") return "JPG, PNG and WebP input. Each file keeps its own format and is re-encoded to strip EXIF/GPS/camera metadata, then the whole batch downloads as one ZIP.";
...
const RAW_IMAGE_TOOLS: ImgTool[] = [
  ...
  { slug: "batch-metadata-remove", title: "Batch Metadata Remover", short: "Batch Strip EXIF", desc: "Strip EXIF/GPS metadata from many photos at once.", emoji: "🛡️", grad: G.green, category: "Batch", engine: "batch:meta", status: "live", isNew: true, keywords: ["batch remove exif", "strip metadata from multiple photos", "bulk exif remover"], intro: "Strip EXIF/GPS metadata from many photos at once.", about: "Queue a whole folder of photos and re-encode every one to remove embedded EXIF, GPS and camera data — each file keeps its own format, and the clean copies download in one ZIP.", steps: S3("Add multiple images.", "Metadata is stripped on export.", "Download the clean ZIP."), faqs: baseFaq() },
