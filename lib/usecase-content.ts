...
export const USE_CASES_EN: UseCaseSeed[] = [
  ...
  },
  {
    ...
    content: {
      ...
      metaDescription: "Shrink a PDF that's too big to email. Reduce the file size in your browser — no upload, no size cap, no watermark. Free and private.",
      intro: "When a PDF is too large to email, compressing it reduces the file size while keeping it readable. QRix recompresses the photos inside the document — which is where nearly all the weight sits in a scan or an export — and does it in your browser, so the document never leaves your device.",
      benefits: [
        "Get under Gmail/Outlook's 25 MB limit without splitting the file.",
        "Runs on your device — sensitive documents never upload, and there is no size limit to hit.",
        "Text, links and form fields are left exactly as they were; only the images are re-encoded.",
      ...
      faqs: [
        ...
        { q: "Will the text still be readable?", a: "Yes — text is never touched. It is stored as vectors and stays as sharp as the original at any setting; only the embedded photos are re-encoded." },
        { q: "Is there a file limit?", a: "Only your device's memory. A 50 MB scan is fine on a normal laptop or phone — it is your browser doing the work, not an upload." },
        { q: "Why did my file barely shrink?", a: "Because it was already lean. The saving comes from re-encoding embedded photos, so a text-only or vector PDF has almost nothing to give up — the tool hands back the original rather than a worse copy of it." },
      ],
    },
  },
  {
