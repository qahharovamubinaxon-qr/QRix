...
- Next.js App Router + TS + Tailwind v4. Palette = orange `#ff4d1c` (primary) · black · green `#467434` (secondary); lilac `#bba9ff` survives only as the image-category accent. Tokens in `app/globals.css` + `app/design-v2.css`. Display font **Bricolage Grotesque** (`--font-display`); `Space Mono` for `.qx-mono`; SUSE/Poppins/Unbounded are used by the canvas tools only.
...
- **Build note:** `next build` DOES complete on this machine — it was run repeatedly on 2 Sep 2026, exit 0 each time. The old advice to skip it was out of date and cost real coverage. It can still die with `Zone Allocation failed - process out of memory` when the machine is under load or when a module reaches out to the network at build time (one did: a Supabase call in `lib/server/db.ts`, now guarded on `NEXT_PHASE`). If it OOMs, close what else is running and try again before assuming the code is at fault. `tsc --noEmit` remains the fast gate, but it is not a substitute — it never catches a prerender that fails.
